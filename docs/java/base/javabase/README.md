...
### **12. 值传递和引用传递有什么区别？**
**值传递**： 方法接收的是实参的真实值，在调用函数时将实际参数的**值复制**一份传递到函数中。  
**引用传递**：方法接收的是实参的内存地址，形参和实参此时都引用了一块堆内存（关于堆内存会在之后的JVM内存划分中详细整理）。在调用函数时将实际参数的**地址传递**到函数中。
...
*PS. 关于这个问题网上一直有很多种说法，有的人认为引用类型虽然把引用地址传递给了形参，但当形参被重新初始化赋值时，实参的引用地址并没有被改变，所以它也是一种值传递（参见[这里](https://github.com/hollischuang/toBeTopJavaer/blob/master/basics/java-basic/java-pass-by.md)）。我觉得也不无道理，这个问题没有标准答案，大家自行比较～*